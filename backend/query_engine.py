...
class QueryEngine:
    ...
    def __init__(self, graph_adapter, entities: Dict[str, List[Dict]]):
        self.graph = graph_adapter  # GraphInterface (Neo4jAdapter or NetworkXAdapter)
    ...
    def _enhance_with_llm(self, original_query: str, graph_result: Dict) -> Dict:
        ...
        if not raw_data or raw_data == 'None':
            import re
            # Extract document IDs from the original query
            doc_match = re.search(r'(\d{8,})', original_query)
            if doc_match:
                doc_id = doc_match.group(1)
                return {
                    'answer': f"⚠️ Document **{doc_id}** was not found in the dataset.\n\nPlease verify:\n- The document number is correct\n- The document exists in the loaded data\n- You're using the right document type (Sales Order, Delivery, Invoice, etc.)",
                    'query_type': 'error',
                    'data': None,
                    'highlighted_nodes': []
                }
...
Respond ONLY with the formatted answer. Do not include any preamble like "Here is..." or "Based on the data..."."""
    ...
    def _try_graph_query(self, query: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Customer query - specific customer sales orders
            elif 'customer' in query_lower and 'sales order' in query_lower and not ('most' in query_lower or 'top' in query_lower):
                result = self._execute_specific_customer_orders_query(query)
                if result:
                    return {
                        'answer': result.get('answer', ''),
                        'query_type': 'search',
            ...
            # Customer query - top customers by order count
            elif 'customer' in query_lower and ('most' in query_lower or 'top' in query_lower):
...
"""
    ...
    def _execute_trace_query(self, query: str) -> Optional[Dict]:
        ...
        if not start_node or not self.graph.has_node(start_node):
            print(f"TRACE ERROR: Node '{start_node}' not found in graph for query: {query}")
            return None  # Don't return random data - let the system handle "not found"
    ...
    def _execute_specific_customer_orders_query(self, query: str) -> Optional[Dict]:
        """Find sales orders for a specific customer by ID or name"""
        import re
        
        # Extract customer ID or name from query
        cust_id_match = re.search(r'(\d{9,10})', query)  # Match customer ID like 320000083 or 3230000083
        cust_match = re.search(r'CUST_\d+', query, re.IGNORECASE)
        ...
        # Try different patterns to find the customer
        if cust_match:
        ...
        customer_data = self.graph.nodes[customer_id]
        customer_label = customer_data.get('businessPartnerFullName', customer_data.get('label', customer_id))
        
        # Find all sales orders for this customer
        sales_orders = []
        highlighted = [customer_id]
        
        for succ in self.graph.successors(customer_id):
            if succ.startswith('SO_'):
                so_data = self.graph.nodes[succ]
                sales_orders.append({
                    'id': succ,
                    'salesOrder': so_data.get('salesOrder', ''),
                    'creationDate': so_data.get('creationDate', 'N/A'),
                    'totalNetAmount': so_data.get('totalNetAmount', '0'),
                    'currency': so_data.get('transactionCurrency', 'INR'),
                    'deliveryStatus': so_data.get('overallDeliveryStatus', ''),
                    'billingStatus': so_data.get('overallOrdReltdBillgStatus', '')
                })
                highlighted.append(succ)
        
        if not sales_orders:
            results = f"No sales orders found for customer {customer_label} ({customer_id})."
            answer = results
        else:
            # Sort by creation date (most recent first)
            sales_orders.sort(key=lambda x: x['creationDate'] or '', reverse=True)
            
            results = f"Sales Orders for Customer: {customer_label}\n"
            results += f"Customer ID: {customer_id}\n"
            results += f"Total Orders: {len(sales_orders)}\n\n"
            results += f"{'Sales Order':<15} {'Date':<12} {'Amount':<15} {'Delivery':<10} {'Billing':<10}\n"
            results += "-" * 70 + "\n"
            
            for so in sales_orders[:20]:  # Limit to first 20
                date_str = so['creationDate'][:10] if so['creationDate'] != 'N/A' else 'N/A'
                amount_str = f"{so['totalNetAmount']} {so['currency']}"
                delivery_str = so['deliveryStatus'] if so['deliveryStatus'] else 'N/A'
                billing_str = so['billingStatus'] if so['billingStatus'] else 'N/A'
                results += f"{so['salesOrder']:<15} {date_str:<12} {amount_str:<15} {delivery_str:<10} {billing_str:<10}\n"
            
            if len(sales_orders) > 20:
                results += f"\n... and {len(sales_orders) - 20} more orders"
            
            answer = f"Customer '{customer_label}' ({customer_id}) has {len(sales_orders)} sales order(s). Details are shown in the data table."
        
        return {
            'results': results,
            'highlighted_nodes': highlighted[:15],  # Limit highlights
            'answer': answer
        }
